...
//! Once `mongol_convert::translate` lands, `parity_against_java_oracle` flips on (see the ignored test).
...
fn parity_implemented_paths() {
    ...
    for r in &rows {
        ...
        let from: mongol_convert::CodeType = r.from.parse().unwrap();
        let to: mongol_convert::CodeType = r.to.parse().unwrap();
        let got = mongol_convert::translate(from, to, &r.input).unwrap_or_else(|e| {
            panic!(
                "{} -> {} errored ({e}) on input {}",
                r.from,
                r.to,
